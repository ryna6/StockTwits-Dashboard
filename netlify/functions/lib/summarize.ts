...
function firstSentenceFor(messages: MessageLite[], fallback: string): string {
  const best = messages[0];
  if (!best) return fallback;
  const txt = (best.body || "").replace(/\s+/g, " ").trim();
  return txt ? txt.slice(0, 140) : fallback;
}
...
export function build24hSummary(args: {
  ...
  const themes = topThemes(args.cleanMessages);
  ...
  if (strong.length < 3 && !hasHighConfidenceCluster) {
    return { longSummary: EMPTY_RECAP, themes, evidencePosts: [] };
  }

  const primaryThread = themes[0]?.name ?? "mixed catalyst talk";
  const secondaryThread = themes[1]?.name ?? "follow-up filings/compliance updates";

  const keyEvidence = firstSentenceFor(args.highlights, "No clear key-user confirmation post yet");
  const broadEvidence = firstSentenceFor(strong.map((x) => x.m), "Broader flow remained fragmented");
  ...
  return { longSummary, themes, evidencePosts };
}
